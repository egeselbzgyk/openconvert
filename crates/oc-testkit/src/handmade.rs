...
// ---------------------------------------------------------------------------
// The benchmark's reference book
// ---------------------------------------------------------------------------

/// The page box the reference book uses: a trade paperback at 6 x 9 inches, in points.
const BOOK_PAGE: [f32; 4] = [0.0, 0.0, 432.0, 648.0];

/// How many body lines a reference page carries. Twenty-eight lines of ten-ish words is
/// roughly 2 000 characters a page, which is what a set trade book actually holds.
const BOOK_LINES_PER_PAGE: usize = 28;

/// Where the first body line sits, and how far apart the lines are.
const BOOK_FIRST_BASELINE_Y: f32 = 576.0;
const BOOK_LEADING_PT: f32 = 15.0;
const BOOK_LEFT_MARGIN_PT: f32 = 54.0;

/// The running head and the folio, at the top and bottom of every page. They are here because
/// a book without them does not exercise furniture detection, and furniture detection is a
/// cross-page pass whose cost is the reason a benchmark runs on three hundred pages rather
/// than on three.
const BOOK_HEAD_Y: f32 = 612.0;
const BOOK_FOLIO_Y: f32 = 36.0;
const BOOK_RUNNING_HEAD: &str = "A Schedule of Rates";

/// A chapter opens every this many pages, so the structure stage has boundaries to find.
const BOOK_PAGES_PER_CHAPTER: usize = 20;
const BOOK_CHAPTER_SIZE_PT: f32 = 18.0;

/// The sentences the body is built from. Eight of them, cycled with the line number, so the
/// text is neither one repeated line — which furniture detection would eat — nor random,
/// which would make the benchmark's input differ between runs.
const BOOK_SENTENCES: [&str; 8] = [
    "The office was quiet at that hour and the clerk did not look up",
    "He was copying a schedule of freight rates in a regular hand",
    "There is a kind of patience that is really a kind of pressure",
    "The rates were those of the spring schedule superseded in June",
    "A second paragraph runs on for several lines and ends the page",
    "The visitor waited by the door and counted the ledger columns",
    "Nothing whatever could be done before the schedule was finished",
    "By six o'clock the office clock had run four minutes slow again",
];

/// A born-digital book of `pages` pages, for the performance budget of D13.11 and
/// IMPLEMENTATION_PLAN PHASE 7 rows 7.11 and 7.12.
///
/// Built rather than committed: three hundred pages of prose is a megabyte of fixture that
/// would be regenerated by nobody and reviewed by nobody. It is deterministic — the same
/// `pages` gives the same bytes — so a benchmark measures the pipeline rather than the input.
///
/// It carries what makes a book expensive to convert, not just long: a running head and a
/// folio on every page for furniture detection to find and remove, a chapter opening every
/// twenty pages for the structure stage, and body lines at a real leading so paragraph
/// reconstruction has real line spacing to reason about.
pub fn reference_book(pages: usize) -> Vec<u8> {
    let built = (0..pages).map(reference_page).collect();
    build_pages(built)
}

fn reference_page(index: usize) -> Page {
    let mut page = Page::default()
        .media_box(BOOK_PAGE)
        .text((BOOK_LEFT_MARGIN_PT, BOOK_HEAD_Y), BOOK_RUNNING_HEAD);

    let mut baseline = BOOK_FIRST_BASELINE_Y;
    let mut line = 0;

    if index.is_multiple_of(BOOK_PAGES_PER_CHAPTER) {
        let chapter = index / BOOK_PAGES_PER_CHAPTER + 1;
        page = page.text_at(
            (BOOK_LEFT_MARGIN_PT, baseline),
            &format!("Chapter {chapter}"),
            BOOK_CHAPTER_SIZE_PT,
        );
        baseline -= BOOK_LEADING_PT * 2.0;
        line += 1;
    }

    while line < BOOK_LINES_PER_PAGE {
        let sentence = BOOK_SENTENCES[(index + line) % BOOK_SENTENCES.len()];
        page = page.text((BOOK_LEFT_MARGIN_PT, baseline), sentence);
        baseline -= BOOK_LEADING_PT;
        line += 1;
    }

    // The folio, as a book prints it: the page's own number and nothing else.
    page.text(
        (BOOK_LEFT_MARGIN_PT, BOOK_FOLIO_Y),
        &format!("{}", index + 1),
    )
}
