...
/// Row 13.22 (D16, R10 §6.14). OCR has no path to a language model: neither OCR module names the
/// AI crate or a provider, `oc-core` (where the engine adapter lives) does not depend on `oc-ai`,
/// and a scanned book's conversion records no model decision in `ingest`.
#[test]
fn ocr_never_calls_the_llm() {
    ...
    assert!(
        !built
            ...
            .decisions
            .iter()
            .any(|decision| decision.stage == "ingest" || decision.llm.is_some()),
        "{:?}",
        built.document.decisions
    );
}
