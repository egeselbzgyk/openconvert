...
D12 carry dated amendments):

- [x] **Windows + Linux only; macOS in a later 1.x** (D12 amendment). `release.yml` has no macOS leg,
      rows 15.1–15.4 wait for macOS, `xtask::release::SHIPPED = [Linux, Windows]` drives row 15.13;
      signing dry run without macOS; `packaging/macos/` kept — `the_1_0_release_ships_windows_and_linux_only`
- [x] **Validation pack deferred past v1.0; VD-f deferred with it** (D6 amendment). `packs.toml`
      `[[deferred]]` (no pins); desktop refuses (`UiError::NotOffered`), Packs screen "Arrives in a later
      version", `openconvert model pull validation` → exit 2 `E_PACK_NOT_OFFERED` —
      `the_validation_pack_is_deferred_past_1_0`, `the_shipped_validation_pack_is_deferred_past_1_0`,
      `the_validation_pack_is_refused_with_a_clear_message`, the Packs-route UI test
- [x] **AppImage budget 120 MB** — `release.max_linux_installer_bytes` (`binary`, maintainer decision;
      D17 allows no other non-provisional word); `installer_budget(os)`; the updater's payload cap is now
      this OS's budget (it would have refused every AppImage update) —
      `the_installer_budget_counts_installers_only`, `an_update_payload_may_be_as_large_as_this_os_installer`
- [x] **Updater key generated by the maintainer off this machine**; its public key (key id
      `0C6C69CA122C11B0`) replaces `TODO_UPDATER_PUBKEY`; the steps are in RELEASE_CHECKLIST —
      `the_shipped_updater_key_is_the_maintainers_minisign_key`
- [x] **Version 1.0.0** in every manifest; `Cargo.lock`/`fuzz/Cargo.lock` regenerated by cargo;
      `bump-rules-check --tag` also checks `tauri.conf.json` — `the_tag_is_the_version_both_manifests_declare`
- [x] `docs/CHANGELOG.md` `## [1.0.0]` with "Known limitations" (and a release-prep section);
      `README.md` rewritten for the public repository; `docs/INSTALL.md`; RELEASE_CHECKLIST run
      (2026-09-23, release preparation)

**Gates (2026-09-23, on the branch):** `cargo fmt --check` clean; workspace clippy (`--exclude
openconvert-desktop --all-targets --all-features -D warnings`) clean; `cargo nextest run --workspace
--exclude openconvert-desktop` **840 tests, all green**; desktop clippy (all features;
`--no-default-features`) clean and 66 tests green with `engine-integration`; UI Vitest 58, lint, check
clean; `cargo tauri dev` starts the app (xvfb); `ci-lint`, **`ci-lint --release-branch` clean (no
placeholder left)**, `thresholds-lint`, `bump-rules-check --tag v1.0.0`, `notices --check` clean; `cargo
test -p xtask --locked --test release` 27 passed; `cargo deny --all-features check` and the tooling
policy clean.

**Release state.** Appendix D does not fully pass (below, unchanged except where these decisions close
an item); the maintainer chose to ship 1.0.0 with the listed limitations. `STATUS` stays `IN_PROGRESS`:
Phase 7.5 and the other open items continue after 1.0. The maintainer's remaining steps: the two signing
secrets, the updater endpoint question (`## Blocked`), then `git tag v1.0.0 && git push origin v1.0.0`;
after the release, `bump-rules-check --record v1.0.0` and `docs/releases/1.0.0/`.
...
| Item | State |
|---|---|
| VD-a … VD-g closed or deferred past v1 with the reason | **Yes, since the release preparation.** VD-a, VD-b, VD-d, VD-g closed. VD-c, VD-e deferred past v1 (they block only the post-v1 dictionary pack; PROVISIONAL, DECISIONS_LOG 2026-09-23 "Appendix D evaluated"). **VD-f deferred past v1 by the maintainer (2026-09-23)** with the validation pack itself (D6 amendment): no pack, so no vendor whose licence could be read. |
...
| Item | State |
|---|---|
| macOS: every nested Mach-O signed, notarized, stapled; `spctl` accepts | **Not in 1.0** — the maintainer moved macOS to a later 1.x (2026-09-23, D12 amendment). `sign_nested.sh` and `notarize.sh` stay, tested on Linux with a recording `codesign`. |
...
| Updater manifest signed; tampered payload refused | **Partly** — the verifier accepts a signed manifest and refuses a tampered payload (15.9/15.10, in-test key); the maintainer's public key is in the tree since 2026-09-23 (key id `0C6C69CA122C11B0`); a manifest signed with it exists once the release job runs. |
...
| No `TODO_` in `models.toml`; no expired `review_by` | **Yes** — `models.toml`'s 8 pins filled 2026-09-23 (`fix/phase-09-registry-pins`; the default's download verified, the other three not downloaded). No expired `review_by` (`thresholds-lint` clean). Since the release preparation `ci-lint --release-branch` finds **nothing** (validation pack deferred without pins; the updater key filled). |
...
AppImage is 112 953 848 bytes; since the maintainer's decision (2026-09-23) its budget is 120 000 000.

### Phase 15 — release blockers (need the maintainer)

*Superseded 2026-09-23 by the release preparation (above): the updater key is in the tree, the
validation pack is deferred without pins, the AppImage has a 120 MB budget — `ci-lint
--release-branch` is clean and row 15.15 passes on the measured AppImage. Kept as the record of what
Phase 15 handed over.*
...
1. ~~**VD-f is open**~~ **Closed by the maintainer (2026-09-23):** the validation pack is deferred past
   v1.0 (D6 amendment), and VD-f with it — there is no vendor to read a licence from until the pack is
   built.
...
6. **Release:** *macOS is out of 1.0 (maintainer, 2026-09-23; D12 amendment), the updater public key is
   in the tree, and `packs.toml` has no pin left — `ci-lint --release-branch` is clean.* Still open until
   the tag runs `release.yml`: the Windows installers never built, `flatpak-builder-lint` not run, no
...
8. **CI** — GitHub Actions now runs `ci.yml` on pushes to `main` (run #44 green on ubuntu/macOS/Windows);
   the nightly and release workflows have not run yet, so their rows stay unverified until they do.

**v1.0.0 ships with items 2–5 open, by the maintainer's decision (2026-09-23):** they are the 1.0.0
