...
## 2026-09-23 · One bundle shape: two sidecars, their natives beside them · Phase 15 (P15.2)
Context: PHASE 15 detail 1 asks for the same layout on all three OSes — the engine and
`llama-server` as `externalBin` (D8, D12), `libpdfium` bundled, `models.toml` + `thresholds.toml` as
resources. Two facts shape how. The official `llama-server` (`b10456`) is not self-contained: it
links `libllama-server-impl`, `llama-common`, `llama`, `mtmd`, `ggml`, `ggml-base` with `RUNPATH
$ORIGIN` / `@loader_path` / the DLL search order, and ggml loads its per-ISA CPU backends from the
executable's own directory. And Tauri places `externalBin` beside the app (`usr/bin` in the AppImage,
`Contents/MacOS`, the Windows install directory) while resources go elsewhere on Linux and macOS
(`usr/lib/<product>`, `Contents/Resources`).
Decision: `stage-sidecars` stages `bin/openconvert-engine-<triple>`, `bin/llama-server-<triple>`,
`bin/native/` (PDFium plus the server's runtime, chosen by library family: the six linked libraries,
every `ggml-cpu*` backend, `ggml-metal`/`ggml-blas` on macOS, `libomp140` on Windows — never
`ggml-rpc`, whose job is sending tensors to another machine, and never another tool's `*-impl`) and
`bin/licenses/` (PDFium's BSD-3 and llama.cpp's MIT texts, which both licences require in a binary
distribution). Per-OS files put `bin/native/` *beside* the sidecars: `bundle.linux.appimage.files
{"/usr/bin/": …}`, `bundle.macOS.files {"MacOS/": …}`, and on Windows a resource map to the resource
root, which is the install directory. Beside is where the engine already looks for PDFium first, so
the bundled engine needs no environment to find it. Installers per OS: AppImage; `.app` + `.dmg`;
NSIS + MSI (per-user NSIS, EN/DE/TR). **PROVISIONAL — needs maintainer ratification**: (1) the
`.deb`/`.rpm` D12 calls best-effort are *not* built: Tauri installs `externalBin` into `/usr/bin`,
so a package would put `llama-server` (and, with this layout, its libraries) into the system
`/usr/bin`, where a distribution's own llama.cpp package owns the same path; shipping an artefact
that can collide with system files is worse than not shipping it. (2) The updater's public key in
`tauri.conf.json` is the placeholder `TODO_UPDATER_PUBKEY` — the keypair has to be generated by the
maintainer with the private half going straight into CI secrets (detail 5), never on this machine;
the release gate refuses the placeholder.
Evidence: `the_bundle_layout_is_the_same_on_every_os`,
`the_llama_runtime_is_the_server_and_what_it_loads_on_every_os`,
`the_rpc_backend_and_other_tools_are_never_bundled`; the three release archives the lock pins were
